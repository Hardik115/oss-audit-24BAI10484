...
|--------|----------|---------|
| Script 1 | `script1_system_identity.sh` | Displays a system welcome screen with kernel, user, uptime, and license info |
| Script 2 | `script2_package_inspector.sh` | Checks if Git is installed, shows version and license, prints a philosophy note |
| Script 3 | `script3_disk_auditor.sh` | Audits key Linux directories for permissions, ownership, and disk usage |
| Script 4 | `script4_log_analyzer.sh` | Reads a log file line by line and counts keyword occurrences |
| Script 5 | `script5_manifesto.sh` | Interactively generates a personalised open source philosophy statement |
...
```bash
./script4_log_analyzer.sh /var/log/messages error      # Fedora/RHEL
./script4_log_analyzer.sh /var/log/auth.log error      # Ubuntu
./script4_log_analyzer.sh /var/log/kern.log error      # Kernel log
```
